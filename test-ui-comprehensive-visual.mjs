...
(async () => {
  ...
  const logoExists = await page.locator('header div[role="img"][aria-label="Azune Logo"]').count() > 0;
  report("Header Logo 元素存在", logoExists);

  const logoSvg = await page.locator('header svg').count() > 0;
  report("Header Logo SVG 存在", logoSvg);
  ...
  if (await themeButton.isVisible().catch(() => false)) {
    ...
    const logoInLight = await page.locator('header div[role="img"][aria-label="Azune Logo"]').count() > 0;
  ...
  const mobileLogo = await mobilePage.locator('header div[role="img"][aria-label="Azune Logo"]').count() > 0;
  ...
  // 验证 Logo aria-label
  const logoAria = await a11yPage.locator('header div[role="img"][aria-label="Azune Logo"]').count() > 0;
  report("Logo ARIA 标签", logoAria);
