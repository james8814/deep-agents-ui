...
(async () => {
  ...
  try {
    ...
    report(
      "Light mode CSS 变量 --color-cyan",
      cssVars.cyan?.toLowerCase() === "#38bdf8",
      cssVars.cyan
    );
    report(
      "Light mode CSS 变量 --color-primary",
      cssVars.primary === "#6558d3",
      cssVars.primary
    );
  } catch (err) {
  ...
  try {
    ...
    // 点击主题切换按钮
    const themeButton = await themePage
    ...
    if (await themeButton.isVisible().catch(() => false)) {
      ...
      // 截图记录切换后的状态
      await themePage.screenshot({
        path: "test-ui-logo-theme-switched.png",
        fullPage: true,
      });
  ...
  console.log("\n" + "=".repeat(70));
  console.log(
    ` 真实 UI 测试结果: ${passCount} PASS / ${failCount} FAIL / ${
      passCount + failCount
    } TOTAL`
  );
  console.log("=".repeat(70));
  ...
  fs.writeFileSync(
    "test-logo-ui-real-results.json",
    JSON.stringify(
      {
