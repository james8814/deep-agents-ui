...
describe('shouldAddAuthHeader', () => {
  ...
  describe('LangGraph Server 匹配', () => {
    ...
    test('localhost:2025 应匹配', () => {
      expect(shouldAddAuthHeader('http://localhost:2025/assistants')).toBe(true);
    });
    ...
    test('使用环境变量配置的 API URL 应匹配', () => {
      ...
      expect(shouldAddAuthHeader('https://api.example.com/assistants')).toBe(true);
    });
  ...
  describe('Auth Server /api/ 端点匹配', () => {
    test('localhost:8000 的 /api/ 端点应匹配', () => {
      ...
      expect(shouldAddAuthHeader('http://localhost:8000/api/upload')).toBe(true);
    });
    ...
    test('使用环境变量配置的 Auth URL 的 /api/ 端点应匹配', () => {
      ...
      expect(shouldAddAuthHeader('https://auth.example.com/api/files')).toBe(true);
    });

    test('/auth/ 端点不应匹配（登录/注册本身不需要 token）', () => {
      expect(shouldAddAuthHeader('http://localhost:8000/auth/login')).toBe(false);
      expect(shouldAddAuthHeader('http://localhost:8000/auth/register')).toBe(false);
      expect(shouldAddAuthHeader('http://localhost:8000/auth/logout')).toBe(false);
    });
  ...
  describe('URL 包含检查', () => {
    test('URL 包含端口号时应匹配（使用环境变量配置时）', () => {
      ...
      expect(shouldAddAuthHeader('https://api.example.com:8080/threads')).toBe(true);
    });

    test('URL 包含子路径时应匹配', () => {
      expect(shouldAddAuthHeader('http://localhost:2024/v1/threads/abc123')).toBe(true);
      expect(shouldAddAuthHeader('http://localhost:2024/threads/abc123/runs')).toBe(true);
    });

    test('带查询参数的 URL 应正确匹配', () => {
      expect(shouldAddAuthHeader('http://localhost:2024/threads?limit=10')).toBe(true);
      expect(shouldAddAuthHeader('http://localhost:8000/api/files?id=123')).toBe(true);
    });
  });
