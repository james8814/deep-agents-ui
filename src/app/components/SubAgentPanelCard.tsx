    ...
    <div className="mb-3 overflow-hidden rounded-[var(--r-md)] border border-[var(--b1)] bg-[var(--bg2)]">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 flex-1 items-center gap-2">
            ...
            <div className="min-w-0 flex-1">
              <div className="truncate font-medium text-[var(--t1)]">
                {agent.name}
              </div>
      ...
      {expanded && (
        <div className="space-y-3 border-t border-[var(--b1)] p-3">
          ...
          {(agent.toolCalls?.length ?? 0) > 0 && (
            <div>
              <div className="mb-2 text-xs font-semibold text-[var(--t2)]">
                工具调用
              </div>
          ...
          {agent.error && (
            <div className="rounded-[var(--r-sm)] bg-[var(--err)] bg-opacity-10 p-2">
              <div className="mb-1 text-xs font-medium text-[var(--err)]">
                错误
              </div>
              <div className="break-words font-mono text-xs text-[var(--t2)]">
                {agent.error}
              </div>
            </div>
          )}
          ...
          {(agent.startTime || agent.endTime) && (
            <div className="space-y-1 text-xs text-[var(--t3)]">
              {agent.startTime && (
                <div>
                  开始: {new Date(agent.startTime).toLocaleTimeString()}
                </div>
              )}
              {agent.endTime && (
      ...
      {call.tool_input && (
        <div>
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-[var(--t2)] transition-colors hover:text-[var(--t1)]"
          >
            <ChevronDown
              className={`h-3 w-3 transition-transform ${
                expandedInput ? "rotate-180" : ""
              }`}
            />
            输入参数
          </button>
