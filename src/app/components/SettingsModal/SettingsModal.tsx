        ...
        <Dialog.Portal>
          ...
          <Dialog.Content
            className={cn(
              ...
              'rounded-lg border border-azune-border-1',
          ...
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-azune-border-1 px-6 py-4">
              <div>
                <Dialog.Title
                  id="settings-modal-title"
                  className="text-lg font-semibold text-azune-text-1"
                >
                  Settings
                </Dialog.Title>
                <p
                  id="settings-modal-description"
                  className="text-sm text-azune-text-3"
                >
              ...
              <Dialog.Close asChild>
                <button
                  className={cn(
                    ...
                    'focus:outline-none focus-visible:ring-2 focus-visible:ring-azune-brand focus-visible:ring-offset-2',
            ...
            {/* Tabs */}
            <div className="border-b border-azune-border-1">
              <div className="flex">
                {(['appearance', 'notifications', 'shortcuts', 'about'] as const).map(
                  tab => (
                    <button
                      key={tab}
                      ...
                      className={cn(
                        ...
                        state.activeTab === tab
                          ...
                          : 'border-transparent text-azune-text-2 hover:text-azune-text-1'
                      )}
                      role="tab"
            ...
            <div className="max-h-96 overflow-y-auto px-6 py-6">
              ...
              {state.activeTab === 'appearance' && (
                <div id="settings-appearance" role="tabpanel">
                  <SettingsSection
                    title="Theme"
              ...
              {state.activeTab === 'notifications' && (
                <div id="settings-notifications" role="tabpanel">
                  <SettingsSection
                    title="Notifications"
              ...
              {state.activeTab === 'shortcuts' && (
                <div id="settings-shortcuts" role="tabpanel">
                  <SettingsSection
                  ...
                  >
                    <div className="space-y-4">
                      <input
                        ...
                        className={cn(
                          'w-full rounded-md border border-azune-border-1',
                          'bg-azune-bg-2 px-3 py-2 text-sm',
                          'text-azune-text-1 placeholder-azune-text-3',
                          'transition-colors',
                          'focus:border-azune-brand focus:outline-none focus-visible:ring-2 focus-visible:ring-azune-brand focus-visible:ring-offset-2',
                          'dark:focus-visible:ring-offset-azune-bg-1'
                        )}
                      ...
                      <div className="space-y-2">
                        {filteredShortcuts.length > 0 ? (
                          ...
                          <p className="text-center text-sm text-azune-text-3 py-8">
                            No shortcuts found
                          </p>
                        )}
                      </div>
              ...
              {state.activeTab === 'about' && (
                <div id="settings-about" role="tabpanel">
                  <SettingsSection
                    title="About PMAgent"
                    description="Application information and credits"
                  >
                    <div className="space-y-4">
                      <div className="rounded-md bg-azune-bg-2 p-4">
                        <dl className="space-y-3 text-sm">
                          <div className="flex justify-between">
                            <dt className="text-azune-text-2">Version</dt>
                            <dd className="font-medium text-azune-text-1">
                              {ABOUT_INFO.version}
                            </dd>
                          </div>
                          <div className="flex justify-between">
                      ...
                      <button
                        className={cn(
                          'w-full rounded-md bg-azune-brand px-4 py-2',
            ...
            <div
              className={cn(
                'border-t border-azune-border-1 px-6 py-4',
            ...
            >
              <button
                ...
                className={cn(
                  ...
                  'transition-colors hover:bg-azune-bg-3',
                  'focus:outline-none focus-visible:ring-2 focus-visible:ring-azune-brand focus-visible:ring-offset-2',
                  'dark:focus-visible:ring-offset-azune-bg-1'
                )}
              >
...
const SettingsSection: FC<SettingsSectionProps> = ({
  ...
  <section className="space-y-4">
    <div>
      <h3 className="text-base font-semibold text-azune-text-1">{title}</h3>
      {description && (
        <p className="mt-1 text-sm text-azune-text-3">{description}</p>
      )}
    </div>
    {children}
  </section>
...
const ToggleSetting: FC<ToggleSettingProps> = ({
  ...
  <div
    className={cn(
      ...
      disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-azune-bg-2'
    )}
  >
    <div className="flex-1">
      <label
        htmlFor={id}
        className={cn(
          'block text-sm font-medium text-azune-text-1',
      ...
      {description && (
        <p className="mt-1 text-xs text-azune-text-3">{description}</p>
      )}
    </div>
    ...
    <input
      ...
      disabled={disabled}
      className={cn(
        'mt-1 h-5 w-5 rounded border-azune-border-1',
        'cursor-pointer accent-azune-brand',
        'focus:outline-none focus-visible:ring-2 focus-visible:ring-azune-brand focus-visible:ring-offset-2',
        'dark:focus-visible:ring-offset-azune-bg-1',
...
const ThemeToggle: FC<ThemeToggleProps> = ({ currentTheme, onThemeChange }) => (
  <div className="flex gap-3">
    {(['light', 'dark'] as const).map(theme => (
      ...
      >
        {theme === 'light' ? (
          <Sun className="h-6 w-6 text-azune-text-1" />
        ) : (
          <Moon className="h-6 w-6 text-azune-text-1" />
        )}
        <span className="text-xs font-medium capitalize text-azune-text-1">
          {theme}
        </span>
      </button>
    ))}
...
const ShortcutItem: FC<ShortcutItemProps> = ({
  ...
  <div className="flex items-center justify-between rounded-md bg-azune-bg-2 p-3 text-sm">
    <div className="flex-1">
      <p className="font-medium text-azune-text-1">{shortcut.action}</p>
      <p className="text-xs text-azune-text-3">{shortcut.description}</p>
    </div>

    <div className="flex items-center gap-2">
      <kbd
        className={cn(
          'rounded bg-azune-bg-3 px-2 py-1 text-xs font-mono',
          'text-azune-text-1 border border-azune-border-1'
        )}
      >
        {shortcut.keys.join(' + ')}
      </kbd>
