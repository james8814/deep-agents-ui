...
describe('StatusIndicator Component', () => {
  describe('Rendering', () => {
    it('should render connected status', () => {
      render(<StatusIndicator status="connected" showLabel />);
    ...
    it('should render connecting status', () => {
      render(<StatusIndicator status="connecting" showLabel />);

      expect(screen.getByText('Connecting')).toBeInTheDocument();
    });

    it('should render disconnected status', () => {
      render(<StatusIndicator status="disconnected" showLabel />);

      expect(screen.getByText('Disconnected')).toBeInTheDocument();
    });

    it('should render error status', () => {
      render(<StatusIndicator status="error" showLabel />);

      expect(screen.getByText('Error')).toBeInTheDocument();
    });

    it('should render idle status', () => {
      render(<StatusIndicator status="idle" showLabel />);
  ...
  describe('Visual Variants', () => {
    ...
    it('should handle small size', () => {
      const { container } = render(<StatusIndicator status="connected" size="sm" />);
    ...
    it('should handle medium size', () => {
      const { container } = render(<StatusIndicator status="connected" size="md" />);
    ...
    it('should handle large size', () => {
      const { container } = render(<StatusIndicator status="connected" size="lg" />);
  ...
  describe('Animations', () => {
    it('should pulse when status is connecting and pulse is true', () => {
      const { container } = render(
        <StatusIndicator status="connecting" pulse={true} />
      );
    ...
    it('should not pulse when pulse is false', () => {
      const { container } = render(
        <StatusIndicator status="connected" pulse={false} />
      );
  ...
  describe('Accessibility', () => {
    ...
    it('should have aria-label with custom label', () => {
      ...
      const statusElement = screen.getByRole('status');
      expect(statusElement.getAttribute('aria-label')).toContain('Custom Status');
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle rapid status changes', () => {
      const { rerender } = render(<StatusIndicator status="connected" showLabel />);

      rerender(<StatusIndicator status="disconnected" showLabel />);
      expect(screen.getByText('Disconnected')).toBeInTheDocument();
  ...
  describe('Integration', () => {
    it('should work with different status and size combinations', () => {
      const statuses = ['connected', 'connecting', 'disconnected', 'error', 'idle'] as const;
      const sizes = ['sm', 'md', 'lg', 'xl'] as const;

      statuses.forEach(status => {
        sizes.forEach(size => {
          const { unmount } = render(
            <StatusIndicator status={status} size={size} showLabel />
          );
          expect(screen.getByRole('status')).toBeInTheDocument();
          unmount();
        });
