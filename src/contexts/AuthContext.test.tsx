...
describe('isTokenExpired', () => {
  ...
  test('应返回 true，当 payload 解析失败', () => {
    ...
    const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.!!!invalid!!!.mock-signature';
  ...
  test('应正确处理 URL-safe base64 字符（- 和 _）', () => {
    ...
    const payload = btoa(JSON.stringify({ exp: Math.floor(Date.now() / 1000) + 3600 })).replace(/\+/g, '-').replace(/\//g, '_');
...
describe('AuthProvider', () => {
  ...
  test('登录成功时应存储 token 并设置用户', async () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith('auth_token', mockLoginResponse.access_token);
    expect(mockAuthApi.getUserInfo).toHaveBeenCalledWith(mockLoginResponse.access_token);
  });
  ...
  test('获取用户信息失败（401/403）时应清除 token', async () => {
    const validToken = createMockToken({ exp: Math.floor(Date.now() / 1000) + 3600 });
    (localStorage.getItem as jest.Mock).mockReturnValue(validToken);
    mockAuthApi.getUserInfo.mockRejectedValue(new HttpError('Unauthorized', 401));
...
function TestComponent() {
  const { user, token, isAuthenticated, isLoading, hasChecked, login, logout } = useAuth();
  ...
  return (
    <div>
      ...
      <div data-testid="authenticated">{isAuthenticated ? '已认证' : '未认证'}</div>
...
function renderHookWithAuth() {
  let result: any;
  function HookWrapper({ children }: { children: (result: any) => React.ReactNode }) {
    result = useAuth();
