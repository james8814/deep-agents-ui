...
(async () => {
  ...
  try {
    ...
    // 截图记录
    await darkPage.screenshot({ path: "test-logo-header-dark.png", fullPage: false });
    console.log("  📸 Dark mode 截图: test-logo-header-dark.png");

    // 验证 Logo 元素存在
    const logoExists = await darkPage.locator('div[role="img"][aria-label="Azune Logo"]').count() > 0;
    report("Dark mode Logo 元素存在", logoExists);

    // 验证 Logo 在 header 中
    const logoInHeader = await darkPage.locator('header div[role="img"][aria-label="Azune Logo"]').count() > 0;
  ...
  try {
    ...
    // 截图记录
    await lightPage.screenshot({ path: "test-logo-header-light.png", fullPage: false });
    console.log("  📸 Light mode 截图: test-logo-header-light.png");

    // 验证 Logo 元素存在
    const logoExists = await lightPage.locator('div[role="img"][aria-label="Azune Logo"]').count() > 0;
    report("Light mode Logo 元素存在", logoExists);

    // 验证 Logo 在 header 中
    const logoInHeader = await lightPage.locator('header div[role="img"][aria-label="Azune Logo"]').count() > 0;
  ...
  try {
    ...
    // 截图记录
    await mobilePage.screenshot({ path: "test-logo-header-mobile.png", fullPage: false });
    console.log("  📸 Mobile 截图: test-logo-header-mobile.png");

    // 验证 Logo 在 mobile 下存在
    const logoExists = await mobilePage.locator('div[role="img"][aria-label="Azune Logo"]').count() > 0;
  ...
  // ========================================================================
  console.log("\n" + "=".repeat(70));
  console.log(` Logo Header 集成测试结果: ${passCount} PASS / ${failCount} FAIL / ${passCount + failCount} TOTAL`);
  console.log("=".repeat(70));
  ...
  fs.writeFileSync("test-logo-header-results.json", JSON.stringify({
