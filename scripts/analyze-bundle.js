...
function analyzeNextBuild() {
  ...
  console.log(
    `Status: ${totalJsSize <= BUNDLE_BUDGET.js ? "✓ PASS" : "✗ FAIL"}\n`
  );
  ...
  jsFiles.slice(0, 10).forEach((file, i) => {
    ...
    console.log(
      `  ${i + 1}. ${file.name.padEnd(40)} ${formatBytes(file.size).padStart(
        10
      )} (${percentage}%)`
    );
  });
  ...
  console.log(
    `Status: ${totalCssSize <= BUNDLE_BUDGET.css ? "✓ PASS" : "✗ FAIL"}\n`
  );
  ...
  console.log(
    `Status: ${totalSize <= BUNDLE_BUDGET.total ? "✓ PASS" : "✗ FAIL"}\n`
  );
  ...
  if (jsFiles.length > 0) {
    ...
    if (largestFile.size > 100000) {
      recommendations.push(
        `  3. Largest JS file (${largestFile.name}) is ${formatBytes(
          largestFile.size
        )}`
      );
  ...
  console.log("\n✅ SUMMARY:\n");
  console.log(
    `JavaScript:   ${formatBytes(totalJsSize).padStart(12)} / ${formatBytes(
      BUNDLE_BUDGET.js
    ).padEnd(12)} ${totalJsSize <= BUNDLE_BUDGET.js ? "✓" : "✗"}`
  );
  console.log(
    `CSS:          ${formatBytes(totalCssSize).padStart(12)} / ${formatBytes(
      BUNDLE_BUDGET.css
    ).padEnd(12)} ${totalCssSize <= BUNDLE_BUDGET.css ? "✓" : "✗"}`
  );
  console.log(
    `Total:        ${formatBytes(totalSize).padStart(12)} / ${formatBytes(
      BUNDLE_BUDGET.total
    ).padEnd(12)} ${totalSize <= BUNDLE_BUDGET.total ? "✓" : "✗"}`
  );
  console.log("\n");
