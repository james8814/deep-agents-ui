...
async function createTestUser(user: { username: string; email: string; password: string }) {
...
test.describe('Authentication & Route Protection', () => {
  ...
  test('should show loading state briefly on protected pages', async ({ page }) => {
    const testUser = generateTestUser();
  ...
  test('middleware should enforce route protection at request level', async ({ page }) => {
...
test.describe('Cookie Management', () => {
  ...
  test('should clear auth_token cookie after logout', async ({ page }) => {
    ...
    const logoutButton = page.locator('button:has-text("登出"), button:has-text("Logout"), text=登出, text=Logout');
